...
if __name__ =="__main__":
    ...
    for itr in range(args.runs):
        # itr += args.itrsave
        model_sp = root_path + str(dataset_idx)+"/" + args.ModelNameSave + "/Models"
        ...
        name_model_final = model_sp+ '/' + str(itr) + f'_fi_{token}.pt'
        name_model_bestF1 =  model_sp+ '/' + str(itr) + f'_bt_{token}.pt'

        name_model_valloss =  model_sp+ '/' + str(itr) + f'_valloss_{token}.pt'
        
        model_spR = root_path + str(dataset_idx)+"/" + args.ModelNameSave + "/Results"
        if not os.path.exists(model_spR):
            os.makedirs(model_spR)
            
        training_tsx = model_spR+ '/' + f"log_{token}" + '.txt' 
        
    
        training_tsx_mean = model_spR + '/' + f'{token}_mean' + '.txt'
        
        ce_loss = CrossEntropyLoss()
        dice_loss = DiceLoss(args.num_classes)
        
        
        train_set = isic_loader(path_Data = database_path, train = True)
        
        validate_set   = isic_loader(path_Data = database_path, train = False, Test=False)
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=args.base_lr * 0.01, betas=(0.5, 0.99))

        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=optimizer, T_0=len(train_loader), T_mult=2)
            ...
            for phase in ['train', 'valid']:
                ...
                for batch in tqdm(dataloader):
                    ...
                    if phase == 'train':
                        ...
                        with torch.no_grad():
                            outputs11 = model(x)
                            # calculate the loss 
                            loss_ce = ce_loss(outputs11, y1.long())   
                            loss_dice = dice_loss(outputs11, y1, softmax=True)
                ...
                if phase == 'valid':
                    ...
                    if best_val_loss > epoch_loss:
                        ...
                        torch.save(model.state_dict(), name_model_valloss)
                        best_loss_EPS = epoch
                ...
                print("The best val DSC {} at eps {}" . format(best_Dice,best_EPS ))
                print("The best val LOSS {} at eps {}" . format(best_val_loss,best_loss_EPS ))
    ...
    training_tsx = training_tsx_mean
